...
export class CapacitorAdapter implements DatabaseAdapter {
  ...
  async createEntry(entry: Entry): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
    const now = Date.now();
    const query = 'INSERT INTO entries (id, title, type, content, isCompleted, createdAt, updatedAt, metadata, notebookId, trashedAt, deleted, syncedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
    const values = [entry.id, entry.title, entry.type, entry.content || '', entry.isCompleted ? 1 : 0, entry.createdAt || now, entry.updatedAt || now, entry.metadata ? JSON.stringify(entry.metadata) : null, entry.notebookId || null, entry.trashedAt || null, entry.deleted ? 1 : 0, entry.syncedAt || 0];
    await this.db.run(query, values);
  }

  async updateEntry(entry: Entry): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
    const query = 'UPDATE entries SET title = ?, type = ?, content = ?, isCompleted = ?, updatedAt = ?, metadata = ?, notebookId = ?, trashedAt = ?, deleted = ?, syncedAt = ? WHERE id = ?';
    const values = [entry.title, entry.type, entry.content || '', entry.isCompleted ? 1 : 0, Date.now(), entry.metadata ? JSON.stringify(entry.metadata) : null, entry.notebookId || null, entry.trashedAt || null, entry.deleted ? 1 : 0, entry.syncedAt || 0, entry.id];
    await this.db.run(query, values);
  }

  async deleteEntry(id: string): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
    await this.db.run('UPDATE entries SET deleted = 1, updatedAt = ? WHERE id = ?', [Date.now(), id]);
  }
  ...
  async createNotebook(notebook: Notebook): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
    const now = Date.now();
    const query = 'INSERT INTO notebooks (id, name, icon, createdAt, updatedAt, deleted, syncedAt) VALUES (?, ?, ?, ?, ?, ?, ?)';
    const values = [notebook.id, notebook.name, notebook.icon, notebook.createdAt || now, notebook.updatedAt || now, notebook.deleted ? 1 : 0, notebook.syncedAt || 0];
    await this.db.run(query, values);
  }

  async updateNotebook(notebook: Notebook): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
    const query = 'UPDATE notebooks SET name = ?, icon = ?, updatedAt = ?, deleted = ?, syncedAt = ? WHERE id = ?';
    const values = [notebook.name, notebook.icon, Date.now(), notebook.deleted ? 1 : 0, notebook.syncedAt || 0, notebook.id];
    await this.db.run(query, values);
  }

  async deleteNotebook(id: string): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
    await this.db.run('UPDATE notebooks SET deleted = 1, updatedAt = ? WHERE id = ?', [Date.now(), id]);
  }
  ...
  async deleteNotebookWithCascade(notebookId: string, trashedEntries: Entry[]): Promise<void> {
    ...
    const set: any[] = [
      {
        statement: 'UPDATE notebooks SET deleted = 1, updatedAt = ? WHERE id = ?',
        values: [Date.now(), notebookId]
      }
    ];
